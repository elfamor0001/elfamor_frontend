...
export const Navbar = ({ currentPage, setCurrentPage }) => {
  ...
  // Helper: read guest cart from localStorage and sum qty
  const readGuestCartCount = () => {
    try {
      const raw = localStorage.getItem(GUEST_CART_KEY);
      if (!raw) return 0;
      const items = JSON.parse(raw) || [];
      return items.reduce((total, it) => total + (Number(it.quantity) || 0), 0);
    } catch (e) {
      console.error('readGuestCartCount parse failed', e);
      return 0;
    }
  };

  // Function to fetch cart count from API (for logged-in users) OR from localStorage when logged out
  const fetchCartCount = async () => {
    // If user not logged in, calculate from guest cart in localStorage
    if (!user) {
      const guestCount = readGuestCartCount();
      setDynamicCartCount(guestCount);
      return;
    }
  ...
  // Fetch cart count on component mount and when location changes or user changes
  useEffect(() => {
    fetchCartCount();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [location.pathname, user]);

  // Listen for storage changes (other tabs) and a custom event for same-tab updates
  useEffect(() => {
    const onStorage = (e) => {
      if (e.key === GUEST_CART_KEY || e.key === null) {
        // if guest cart key changed or full clear occurred, recalc
        fetchCartCount();
      }
    };
    ...
    window.addEventListener('guest_cart_updated', onGuestCartUpdated);

    return () => {
      window.removeEventListener('storage', onStorage);
      window.removeEventListener('guest_cart_updated', onGuestCartUpdated);
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [user]);
          ...
          <div className="flex justify-between items-center h-9">
            ...
            <div className="hidden mt-6 md:flex items-center space-x-4 lg:space-x-6">
              ...
              <div className="relative">
                ...
                {dynamicCartCount > 0 && (
                  <span className="absolute -top-1 right-0 transform translate-x-1/2 bg-black text-white text-[10px] font-semibold px-[6px] py-[1px] rounded-full min-w-[18px] h-[18px] flex items-center justify-center z-20">
                    {dynamicCartCount > 99 ? '99+' : dynamicCartCount}
                  </span>
                )}
              </div>
            ...
            <div className="z-50 md:hidden flex items-center px-2 mt-4 space-x-5">
              ...
              <div className="relative">
                ...
                {dynamicCartCount > 0 && (
                  <span className="absolute -top-2 right-0 z-10 bg-black text-white text-[10px] font-semibold px-[5px] py-[1px] rounded-full min-w-[18px] h-[18px] flex items-center justify-center">
                    {dynamicCartCount > 99 ? '99+' : dynamicCartCount}
                  </span>
                )}
              </div>
