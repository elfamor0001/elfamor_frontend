...
function loadGuestCart() {
  try {
    ...
    console.error("loadGuestCart parse failed", e);
    return [];
  }
}
            ...
            <div className="space-y-4 px-2 py-4 lg:p-6 bg-[#efefef] rounded">
              <div className="flex items-start justify-between">
                <div>
                  <h1 className="text-2xl font-bold">{product.name}</h1>
                  {product.volume_ml && <h4 className="text-sm text-gray-400 mt-1">{product.volume_ml}ml</h4>}
                  <div className={`mt-2 text-sm font-medium ${isOutOfStock ? 'text-red-600' : 'text-green-600'}`}>
                ...
                <div className="mt-1 text-right">
                  {discounted > 0 && discounted < price1 ? (
                    <div className="space-y-1">
                      ...
                      <div className="flex flex-col items-end">
                        <div className="flex items-center gap-1">
                          <span className="text-lg lg:text-xl font-bold text-gray-900">₹ {discounted.toLocaleString()}</span>
                        </div>
                        <div className="flex items-center gap-1">
                          <span className="text-xs font-medium text-gray-500 line-through">₹ {price1.toLocaleString()}</span>
                          <span className="text-xs text-red-600 font-semibold">Save ₹ {(price1 - discounted).toLocaleString()}</span>
                        </div>
                      </div>
                    </div>
                  ) : (
                    <div className="text-lg lg:text-xl font-semibold text-gray-900">₹ {price1.toLocaleString()}</div>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                <button
                  onClick={handleAddToCart}
                  disabled={itemActionLoading || isOutOfStock}
                  className={`flex-1 py-3 rounded-md font-semibold transition-colors duration-300 ${isOutOfStock ? 'bg-gray-400 text-gray-200 cursor-not-allowed' : itemActionLoading ? 'bg-gray-500 text-white cursor-not-allowed' : 'bg-black text-white hover:bg-gray-800'}`}
                >
                  {isOutOfStock ? 'OUT OF STOCK' : itemActionLoading ? 'ADDING...' : 'ADD TO CART'}
                </button>

                {!isOutOfStock && (
                  <div className="flex items-center border rounded-md">
                    <button onClick={decrement} disabled={itemActionLoading || quantity <= 1} className={`px-3 py-2 rounded-l-md ${itemActionLoading || quantity <= 1 ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-gray-100 hover:bg-gray-200'}`}>-</button>
                    <div className="px-4 py-2 font-medium">{quantity}</div>
                    <button onClick={increment} disabled={itemActionLoading || quantity >= (product.stock || 10)} className={`px-3 py-2 rounded-r-md ${itemActionLoading || quantity >= (product.stock || 10) ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-gray-100 hover:bg-gray-200'}`}>+</button>
                  </div>
                )}
              </div>
